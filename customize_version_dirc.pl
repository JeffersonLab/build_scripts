...
foreach $href (@b) {
	...
	if (uc($halld_home) ne "NONE") {
	    $writer->emptyTag("package", "name" => "$name", "home" => "$halld_home", "branch" => "dirc_commissioning_2018");
	}
    } elsif ($name eq "halld_recon" && $halld_recon_home) {
	if (uc($halld_recon_home) ne "NONE") {
	    $writer->emptyTag("package", "name" => "$name", "home" => "$halld_recon_home", "branch" => "dirc_commissioning_2018");
	}
    } elsif ($name eq "halld_sim" && $halld_sim_home) {
	if (uc($halld_sim_home) ne "NONE") {
	    $writer->emptyTag("package", "name" => "$name", "home" => "$halld_sim_home", "branch" => "dirc_commissioning_2018");
	}
    } elsif ($name eq "hdds" && $hdds_home) {
	if (uc($hdds_home) ne "NONE") {
	    $writer->emptyTag("package", "name" => "$name", "home" => "$hdds_home", "branch" => "dirc_commissioning_2018");
	}
    } elsif ($name eq "hdgeant4" && $hdgeant4_home) {
	if (uc($hdgeant4_home) ne "NONE") {
	    $writer->emptyTag("package", "name" => "$name", "home" => "$hdgeant4_home", "branch" => "dirc_commissioning_2018");
	}
