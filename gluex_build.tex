...
\subsection{Base-Level Environment Set-Up: {\tt gluex\_env.(c)sh}}\label{section:base-level-environment-setting}
...
\subsection{Cleaning the Environment: {\tt gluex\_env\_clean\_.(c)sh}}

Since {\tt gluex\_env.(c)sh} is sensitive to definitions hanging around in the environment, there is a script provided that will undo all GlueX-related definitions: {\tt gluex\_env\_clean\_.(c)sh}. For the path variables it will only remove the GlueX-related elements leaving the rest.
...
As we saw in Section~\ref{section:base-level-environment-setting},
