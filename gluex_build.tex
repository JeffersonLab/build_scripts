...
Many of the scripts and makefiles described in this note require that the environment variable {\tt BUILD\_SCRIPTS} be defined and point to an instance of this directory.
...
\subsection{Low-Level Environment Set-Up: {\tt gluex\_env.(c)sh}}\label{section:low-level-environment-setting}
...
non-determininistic in the sense that the result depend on the values
...
all GlueX-related definitions: {\tt gluex\_env\_clean.(c)sh}. Sourcing
...
(the user's home directory) will be used in the resulting enviroment.
...
\item{\bf branch}: When using a Git repository, the branch to be
  checked out.\footnote{Not yet implemented; at this writing the
    master branch will always be checked out.}
\item{\bf dirtag}: A string (directory tag) to be appended to the
  standard directory name of the package when it is built. This
  attribute can be used to distinguish different builds of a package
  where the only difference between them is the version(s) of one or
  more prequisite packages. The string used is
  arbitrary.\footnote{There are a lot possible meanings for the
    directory tag. It could mark different combinations of multiple
    prerequisites as well as custom modifications to packages that
    otherwise would have a standard name. Because of the large number
    of possibilities, the form of this string is left to the user.}
\end{description}
...
variables or the URL-defining environment variables to choose the
...
Finally, the script {\tt gluex\_env\_version.(c)sh} combines use of {\tt version.pl} and {\tt gluex\_env.(c)sh} to more conveniently set up the environment. We have already seen an example of its use in Fig.~\ref{figure:version-script}. The script uses {\tt version.pl} just as shown above to set the stage for an invocation of {\tt gluex\_env.(c)sh}.

\subsection{Specifying Build Parameters with a Version File}
\subsubsection{Directory Tags}\label{section:directory-tags}
