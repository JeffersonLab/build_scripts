...
\title{A Version Management System for GlueX \\ \medskip \large GlueX Note 2793-v1}
...
\abstract{A system for building and managing GlueX software is
  ...
  master details of building each of several software packages and
  from the detials of setting up a working environment, while
  maintaining multiple versions of each of several packages
  simultaneously. Particular combinations of package versions can be
  specified succintly in an XML configuration file and this file can
  be used both to guide a complete build of all needed packages and to
  set up the shell environment to use such a build.}
...
to find any prerequistite packages and in addition it affects use of a
...
{\tt version} attribute or the {\tt url} attribute should appear,
never both. If the {\tt url} attribute is used, each package makefile
...
used. Note that for Subversion repositories, branch specification is
done via the URL itself.
...
of one or more prequisite packages. The string used is arbitrary. A
