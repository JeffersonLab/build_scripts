...
RCDB_MAJOR_VERSION=$(echo "$RCDB_VERSION" | awk -F '.' '{printf "%d", $1}')
RCDB_MINOR_VERSION=$(echo "$RCDB_VERSION" | awk -F '.' '{printf "%d", $2}')
if [[ "$RCDB_MAJOR_VERSION" -gt 0 || "$RCDB_MINOR_VERSION" -gt 8 ]]; then export RCDB_SCHEMA_2=true ; else export RCDB_SCHEMA_2=false; fi
